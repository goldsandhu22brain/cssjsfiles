...
function ApplyRemoveProctor(offeringId, isChecked, isProctorLive) {
	...
	$.ajax({
		...
		async: true,
		contentType: 'application/json; charset=utf-8',
		url: '/user/corporate/UpdateAddOns?offeringId=' + offeringId + '&isChecked=' + isChecked + '&proctorMode=' + id,
		dataType: 'html',
		beforeSend: function () {
			$('.ajax-loader').css("visibility", "visible");
		},
...
function RemoveItemFromCart(orderId, offeringId, count) {
	var test = confirm('Are you sure you want to delete from the Cart?');
	if (test == true || test == 'true') {
		MakePostCall('/User/Corporate/UpdateQuantity', { 'orderId': orderId, 'offeringId': offeringId, 'Count': count, 'isAddition': false });
	}
}
...
function ApplyCoupon(Id, couponCode) {
	if (couponCode == null || couponCode == '' || couponCode.len <= 0) {
		...
		MakePostCall('/User/Corporate/ManageDiscountCoupon', { 'orderId': Id, 'CouponName': couponCode });
	}
}

function RemoveCoupon(orderId) {
	MakePostCall('/User/Corporate/ManageDiscountCoupon', { 'orderId': Id });
}

function UpdateQuantity(orderId, offeringId, count) {
	MakePostCall('/User/Corporate/UpdateQuantity', { 'orderId': orderId, 'offeringId': offeringId, 'Count': count, 'isAddition': true });
}
